...
### 需求: 应用日志分类过滤（Issue #63/#64/#65）
**模块:** frontend/theme

在「日志 → 应用日志」中提供“分类过滤”能力，便于在大量后端日志中快速定位问题：
- 过滤输入框支持按分类前缀匹配（基于日志行中的 `[Category]`），并支持逗号分隔（例如 `TUN,Proxy,ConfigSync`）
- 输入框提供 datalist 自动补全（从当前日志内容中动态提取可用分类）
- 过滤条件持久化到 `localStorage`（key: `vea_app_log_filter`），避免每次启动重复设置
- 提供“路径”按钮：一键复制后端返回的 `app.log`/`kernel.log` 路径，便于用户上传调试

**说明:**
- 该过滤仅影响 UI 展示，不影响后端日志写入与留存策略（`app.log`/`kernel.log` 启动轮转，默认保留 7 天）
- 如需查看完整日志，清空过滤条件并刷新即可
...
在「订阅」列表中为每条订阅展示“用量：已用/总量”，来源为 `GET /configs` 返回的 `usageUsedBytes/usageTotalBytes`。当后端未提供用量字段时，显示为 `—`，不依赖“提示用量的假节点”。

#### 场景: 同步后展示用量
- 条件: 订阅同步成功且后端解析到用量字段
- 预期结果: 订阅行状态列展示 `用量：{已用} / {总量}`（单位自动格式化）
